...
export function Header(){
 const {profile,vipAccess,locale,setLocale,currency,setCurrency,t}=useApp();
 ...
 // Family Vault stays invisible unless this account actually has Vault access.
 if(vipAccess)nav.push(["/vip",t.vip]);
 if(profile?.role==="admin")nav.push(["/admin",t.admin]);
 return <header className="site-header"><div className="nav-inner"><Link href="/" className="nav-brand"><Logo compact/></Link><nav className="desktop-nav">{nav.map(([h,l])=><Link key={h} href={h} className={p===h?"navlink active":"navlink"}>{l}</Link>)}</nav><div className="nav-tools"><div className="currency-switch">{(["USD","AMD"] as const).map(c=><button key={c} onClick={()=>setCurrency(c)} className={currency===c?"selected":""}>{c==="USD"?"$":"֏"}</button>)}</div><div className="lang-switch">{(["hy","en","ru"] as const).map(l=><button key={l} onClick={()=>setLocale(l)} className={locale===l?"selected":""}>{l==="hy"?"ՀԱՅ":l==="en"?"ENG":"РУС"}</button>)}</div><Link className="profile-btn" href="/profile">{profile?.photoURL?<img src={profile.photoURL} alt=""/>:<UserRound size={18}/>}</Link><button className="menu-btn" onClick={()=>setOpen(!open)}>{open?<X size={20}/>:<Menu size={20}/>}</button></div></div>{open&&<nav className="mobile-nav">{nav.map(([h,l])=><Link key={h} onClick={()=>setOpen(false)} href={h}>{l}</Link>)}</nav>}</header>
}
